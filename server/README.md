...
转盘与配对报告共用同一套钱包、内容安全与报告链路；转盘只是**入口与仪式感**，报告是**付费深度**。

| 接口 | 说明 |
|---|---|
| `GET /v1/wheel/candidates` | 候选扇区（**服务端过滤，不含向量**）；空池返回 `empty:true` 与 `hint` |
| `GET /v1/wheel/quote` | 转一次多少钱、为什么免/收 |
| `POST /v1/wheel/spin` | 转一次：服务端抽签 + 幂等扣费 |
| `GET /v1/wheel/history` | 我的转动记录 |

页面：`wheel.html` + `js/wheel.js` + `css/wheel.css`。

### 四条硬规则（改代码前先读）

1. **抽签由服务端决定**。`POST /spin` 返回 `{sectors, targetIndex, target}`，前端只负责把动画停到
   `targetIndex`。计划书的写法是前端 `Math.random()` 定 index 再回传 `target_uid` ——
   那样任何人都能构造请求指定想配对的人，还能重放刷次数。静态同步脚本里有一条**反向校验**：
   `js/wheel.js` 一旦出现 `Math.random() * n` 这类写法就直接报错拒绝发布。
2. **候选只取真实 `active` 账号**，`banned`/`invisible` 与未填昵称的一律排除。
   「虚拟头像池」是红线：付费后转到机器生成的假人＝欺诈（消保法 55 条退一赔三）。
3. **先确认有候选，再扣费**。空池返回 `empty:true` 且**不产生任何流水**，
   绝不出现「扣了钱但转盘没东西可指」。
4. **幂等重放返回同一次抽签结果**。抽签有随机性，重放必须一致 —— 否则用户重试会看到不同的人。
   这条有专门的测试用例。

### 为什么候选用轻量打分而不是向量

拉 1024 维向量做全库相似度，意味着**每次转动要取回几百个向量（数 MB）**；而转盘只需要
"挑一个合适的"，语义精算交给报告（那里只有两个人，向量是常数成本）。
候选打分只用标签 Jaccard + 城市 + 年龄带 + 资料完整度，一次转动只取回轻量字段。
规模上限：候选数受 `WHEEL_POOL_LIMIT`（默认 200）约束，万级用户前都够用；再大就该换 ANN 方案。

### 计费

| 项 | 默认 | 环境变量 |
|---|---|---|
| 转一次 | ¥1 | `WHEEL_PRICE`（0 = 免费） |
| 会员每日免费转动 | 3 次 | `MEMBER_DAILY_SPINS` |
| 扇区数 | 12 | `WHEEL_SECTORS` |
| 近期去重 | 排除最近 20 次转到的对象 | `WHEEL_RECENT_EXCLUDE` |
| 单次候选上限 | 200 | `WHEEL_POOL_LIMIT` |

转盘费与报告费**互相独立、不重复收**：转动收 `WHEEL_PRICE`，之后生成报告按 `TIERS` 档位另计
（`basic` 免费）。`e2e-wheel.mjs` 里有断言固定这一点（余额 `50 - 1 - 20 = 29`）。

### ⚠️ 上线前置条件（行政许可，不是工程问题）

转盘把产品属性明确为**婚恋交友**，因此以下两项必须先办：

- **经营性 ICP 许可证**（现有只在工信部做了 ICP 备案，两者不是一回事）
- 微信小程序类目 **「社交-交友」**（需企业主体）；国内安卓商店上架交友类还需《网络文化经营许可证》

办好之前建议保持内测：`ENFORCE_BILLING=0`（不扣款但照常记流水）或限量邀请，不要公开规模化收费。
