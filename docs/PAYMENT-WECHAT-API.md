...
套餐价格：**月卡 ¥29 / 季卡 ¥79 / 年卡 ¥299**（前后端以 `PLANS` 为准）。
...
登录 [微信支付商户平台](https://pay.weixin.qq.com/)（商户号 `1113936939`）：
...
本地自检（不上传密钥，只测签名是否被微信接受）：

```bash
node tools/scripts/test-wechat-sign.mjs
# 期望 HTTP 200 且返回 code_url；若仍 SIGN_ERROR，说明私钥与序列号不是同一套证书
```
